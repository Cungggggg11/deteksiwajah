...
menu = ["HOME","DETEKSI WAJAH", "UPLOAD FOTO", "DATA"] #menu
choice = st.sidebar.selectbox("Menu", menu) #sidebar menu

path = 'gambar' #path to save image
...
if choice == 'DETEKSI WAJAH': 
    st.markdown("<h2 style='text-align: center; color:white;'>IMPLEMENTASI OPENCV FACE RECOGNITION</h2>", unsafe_allow_html=True) #title
    with col1: #column 1
    ...
    if run == True:
        ...
        def faceList(name):
            with open('faceRecognition.csv', 'r+') as f:
...
#register menu
    ...
    image_file = st.file_uploader("Upload foto",type=['png','jpeg','jpg'])
    if image_file is not None:
        file_details = {"FileName":image_file.name,"FileType":image_file.type}
        st.write(file_details)
        img = load_image(image_file)
        with open(os.path.join("gambar",image_file.name),"wb") as f: 
...
#read data menu
    ...
    with col2:
        df = pd.read_csv('faceRecognition.csv')
        st.subheader("READ DATA")
        df = pd.read_csv('faceRecognition.csv')
        st.write(df)
elif choice == 'HOME':
    with col1:
        st.image("gambar.jpeg",width=700,caption="IMPLEMENTASI OPENCV FACE RECOGNITION ") 
